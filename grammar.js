...
module.exports = grammar(C, {
  ...
  rules: {
    _top_level_item: ($, original) =>
      choice(
        ...original.members.filter(
          (member) => member.content?.name != "_old_style_function_definition"
        ),
    ...
    api_macro_identifier: ($) => token(prec(2, /[A-Z][A-Z0-9_]*_API/)),
    ...
    type_qualifier: (_, original) =>
      choice(original, "mutable", "constinit", "consteval"),
    ...
    _class_declaration: ($) =>
      seq(
        ...
        optional(field("api_macro", $.api_macro_identifier)),
    ...
    _declaration_modifiers: ($, original) => choice(original, "virtual"),
    ...
    storage_class_specifier: (_, original) =>
      choice(
    ...
    dependent_type: ($) =>
      prec.dynamic(-1, prec.right(seq("typename", $.type_specifier))),
    ...
    private_module_fragment_declaration: (_) =>
      seq("module", ":", "private", ";"),
    ...
    optional_parameter_declaration: ($) =>
      seq(
        ...
        field(
          "declarator",
          optional(choice($._declarator, $.abstract_reference_declarator))
        ),
        "=",
    ...
    _field_declaration_list_item: ($, original) =>
      choice(
        original,
        $.template_declaration,
    ...
    _constructor_specifiers: ($) =>
      choice($._declaration_modifiers, $.explicit_function_specifier),
    ...
    _function_exception_specification: ($) =>
      choice($.noexcept, $.throw_specifier),
    ...
    _function_postfix: ($) =>
      prec.right(choice(repeat1($.virtual_specifier), $.requires_clause)),
    ...
    noexcept: ($) =>
      prec.right(
        seq("noexcept", optional(seq("(", optional($.expression), ")")))
      ),
    ...
    _namespace_specifier: ($) =>
      seq(optional("inline"), $._namespace_identifier),
    ...
    concept_definition: ($) =>
      seq("concept", field("name", $.identifier), "=", $.expression, ";"),

    // Statements

    _top_level_statement: ($, original) =>
      choice(
        original,
        $.co_return_statement,
        $.co_yield_statement,
    ...
    _expression_not_binary: ($, original) =>
      choice(
        original,
        $.co_await_expression,
        $.requires_expression,
    ...
    _string: ($) =>
      choice($.string_literal, $.raw_string_literal, $.concatenated_string),
    ...
    new_expression: ($) =>
      prec.right(
        ...
        seq(
          ...
          field(
            "arguments",
            optional(choice($.argument_list, $.initializer_list))
          )
        )
      ),
    ...
    new_declarator: ($) =>
      prec.right(
        seq("[", field("length", $.expression), "]", optional($.new_declarator))
      ),

    delete_expression: ($) =>
      seq(optional("::"), "delete", optional(seq("[", "]")), $.expression),
    ...
    requires_clause: ($) =>
      seq("requires", field("constraint", $._requirement_clause_constraint)),
    ...
    requires_expression: ($) =>
      seq(
        ...
        field(
          "parameters",
          optional(alias($.requires_parameter_list, $.parameter_list))
        ),
    ...
    _binary_fold_operator: (_) =>
      choice(
        ...FOLD_OPERATORS.map((operator) =>
          seq(field("operator", operator), "...", operator)
        )
      ),
    ...
    fold_expression: ($) =>
      seq(
        "(",
        choice($._unary_right_fold, $._unary_left_fold, $._binary_fold),
        ")"
      ),

    parameter_pack_expansion: ($) =>
      prec(-1, seq(field("pattern", $.expression), "...")),
    ...
    user_defined_literal: ($) =>
      seq(
        choice($.number_literal, $.char_literal, $._string),
        $.literal_suffix
      ),
